...
@Composable
fun SelesaiPage(navController: NavController, id: String?, context: Context = LocalContext.current) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                Spacer(modifier = androidx.compose.ui.Modifier.padding(20.dp))
            }
            listProduk.forEach {
                Box {
                    Column(
        ...
        if (status.value.equals("selesai")) {
            Button(onClick = {
                ...
                val call2 =
                    retrofit2.UpdateStatus(id!!, StatusDataWrapper(StatusData(status.value)))
                call2.enqueue(
        ...
        } else if (status.value.equals("pending")) {
            Text(text = "Menunggu Konfirmasi")
